...
describe('#compileIamRole', () => {
  ...
  it('should support http:invoke state type', () => {
    serverless.service.stepFunctions = {
      stateMachines: {
        myStateMachine1: {
          id: 'StateMachine1',
          definition: {
            ...
            States: {
              'Get subscription': {
                ...
                Parameters: {
                  'ApiEndpoint.$': "States.Format('https://api.stripe.com/v1/subscriptions/{}', $.data.object.id)",
                  Method: 'GET',
                },
              },
            },
    ...
    expect(statements[0].Action[0]).to.equal('states:InvokeHTTPEndpoint');
  });

  it('should handle permissions for EventBridge connections used in http:invoke', () => {
    serverless.service.stepFunctions = {
      stateMachines: {
        myStateMachine1: {
          id: 'StateMachine1',
    ...
    expect(statements[0].Effect).to.equal('Allow');
    expect(statements[0].Action[0]).to.equal('states:InvokeHTTPEndpoint');
    expect(statements[1].Effect).to.equal('Allow');
    expect(statements[1].Action[0]).to.equal('events:RetrieveConnectionCredentials');
    expect(statements[1].Resource[0]).to.equal('!GetAtt MyConnection.Arn');
    expect(statements[2].Effect).to.equal('Allow');
    expect(statements[2].Action).to.have.same.members([
      'secretsmanager:GetSecretValue',
      'secretsmanager:DescribeSecret',
    ]);
    expect(statements[2].Resource[0]['Fn::Sub']).to.equal('arn:${AWS::Partition}:secretsmanager:${AWS::Region}:${AWS::AccountId}:secret:events!connection/*');
  });
});

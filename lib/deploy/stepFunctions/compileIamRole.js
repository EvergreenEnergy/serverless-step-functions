...
function getHttpInvokePermissions(state) {
  ...
  if (_.has(state, ['Parameters', 'Authentication', 'ConnectionArn'])) {
    permissions.push(
      {
        action: 'events:RetrieveConnectionCredentials',
        resource: [state.Parameters.Authentication.ConnectionArn],
      },
      {
        action: 'secretsmanager:GetSecretValue',
        resource: ['arn:${AWS::Partition}:secretsmanager:${AWS::Region}:${AWS::AccountId}:secret:events!connection/*'],
      },
      {
